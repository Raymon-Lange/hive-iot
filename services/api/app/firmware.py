...
def get_firmware_by_version(version: str) -> dict | None:
    conn = get_connection()
    row = conn.execute(
        "SELECT version, filename, uploaded_at FROM firmware WHERE version = ? ORDER BY id DESC LIMIT 1",
        (version,),
    ).fetchone()
    conn.close()
    if row is None:
        return None
    return {"version": row["version"], "filename": row["filename"], "uploadedAt": row["uploaded_at"]}
